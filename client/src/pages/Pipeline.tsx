...
function PipelineContent({ embedded }: { embedded?: boolean }) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Optimistic update
    setOpportunities((current) =>
      current.map((item) => (item.id === opportunityId ? { ...item, stage: newStage } : item)),
    );

    // API call
    void moveOpportunity(opportunity, newStage);
  };
  ...
  return (
    <div className={`${embedded ? "" : "min-h-screen"} bg-frame-black text-frame-white font-frame-body flex flex-col overflow-x-hidden`}>
        ...
        <section className="flex flex-col xl:flex-row xl:items-end justify-between gap-5 border-b border-frame-gray-3 pb-5">
          ...
          <div className="flex flex-col lg:flex-row gap-2 w-full xl:w-auto">
            <div className="flex flex-col sm:flex-row gap-2 flex-1">
              ...
              <div className="relative min-w-0 sm:w-[190px]">
            ...
            <div className="flex gap-2">
              <ExportButton entityType="opportunities" variant="outline" size="default" />
        ...
        {isLoading ? (
          <section className="flex gap-3 overflow-x-auto pb-4">
            {stages.slice(0, 5).map((stage) => (
              <div key={stage.id} className="flex-shrink-0 w-[280px] border border-frame-gray-3 bg-frame-gray-1/20">
                <div className="px-3 py-2.5 border-b border-frame-gray-3 flex items-center justify-between">
                  <div className="h-4 w-20 bg-frame-gray-3 animate-pulse rounded" />
                  <div className="h-5 w-8 bg-frame-gray-3 animate-pulse rounded" />
                </div>
          ...
          >
            ...
            <DragOverlay>
              {activeOpportunity ? (
                <div className="opacity-50 scale-95">
                  <OpportunityCard
                    opportunity={activeOpportunity}
                    formatCurrency={formatCurrency}
                    formatDate={formatDate}
                    dueTone={getDueTone(activeOpportunity.expected_close_date)}
                    onOpen={() => {}}
                    onEdit={() => {}}
                    onDelete={() => {}}
                    onMove={() => {}}
                    previousStage={previousStage(activeOpportunity)}
                    nextStage={nextStage(activeOpportunity)}
                  />
                </div>
              ) : null}
            </DragOverlay>
      ...
      <ConfirmDialog
        ...
        description="Esta oportunidade será removida permanentemente do pipeline."
        confirmText={t("app.common.delete") as string}
        cancelText={t("app.common.cancel") as string}
        variant="delete"
        isLoading={isSubmitting}
        itemName={selectedOpportunity ? `${selectedOpportunity.title}${selectedOpportunity.client_name ? ` - ${selectedOpportunity.client_name}` : ''}` : undefined}
      />
    </div>
  );
}
...
function DroppableStageColumn({
  ...
  getDueTone: (value?: string | null) => string;
  openDetailModal: (opp: Opportunity) => void;
  openEditModal: (opp: Opportunity) => void;
  openCreateModal: () => void;
  setSelectedOpportunity: (opp: Opportunity) => void;
  setModalMode: (mode: "delete") => void;
  moveOpportunity: (opp: Opportunity, stage: string) => void;
  previousStage: (opp: Opportunity) => PipelineStage | undefined;
  nextStage: (opp: Opportunity) => PipelineStage | undefined;
  t: Translate;
}) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`w-[310px] shrink-0 border ${stage.color} bg-frame-gray-1/10 transition-colors ${
        isOver ? 'ring-2 ring-frame-orange ring-offset-2 ring-offset-frame-black' : ''
      }`}
    >
      <div className="p-3 border-b border-frame-gray-3 bg-frame-black/60">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="text-[0.68rem] text-frame-gray-light">{opportunities.length}</span>
        </div>
        <div className="mt-2 flex items-center justify-between gap-3">
          <p className="text-xs text-frame-gray-light">{stage.description}</p>
          <p className="text-xs text-frame-orange font-semibold">{formatCurrency(totalValue)}</p>
        </div>
      </div>
