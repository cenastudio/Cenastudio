...
import { VideoUploader, type VideoMetadata } from "@/components/video-reviews";
...
function VideoReviewsContent({ embedded }: { embedded?: boolean }) {
  ...
  const handleVideoUploadComplete = useCallback(async (fileId: string, metadata: VideoMetadata) => {
  ...
  const handleCreateReview = async () => {
    ...
    if (!selectedFileId && !rawLink && !uploadedFileId) {
    ...
    try {
      const response = await fetch("/api/video-reviews", {
        ...
        body: JSON.stringify({
          ...
          fileId: uploadedFileId || (selectedFileId ? parseInt(selectedFileId) : null),
          title: title.trim(),
              ...
              <div className="p-4 border-b border-frame-gray-3">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    {showUploader && (
                      <div className="border border-frame-gray-3 p-3 bg-frame-black/40">
                        <VideoUploader
