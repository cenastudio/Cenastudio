...
export default function AllFilesTab() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(isEn ? "File deleted" : "Arquivo excluído");
  ...
  return (
    <div className="space-y-6">
      {/* Storage info */}
      <div className="p-4 border border-frame-gray-3 bg-frame-gray-1/20">
        ...
        <p className="text-xs text-frame-gray-muted mt-2">
          {isEn
            ? `${assets.length} file${assets.length !== 1 ? "s" : ""} across all projects`
            : `${assets.length} arquivo${assets.length !== 1 ? "s" : ""} em todos os projetos`}
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder={isEn ? "Search files..." : "Buscar arquivos..."}
            value={searchQuery}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
          ...
          <h3 className="text-xl font-semibold text-frame-white mb-2">
            {searchQuery || filter !== "all"
              ? (isEn ? "No files found" : "Nenhum arquivo encontrado")
              : (isEn ? "No files yet" : "Nenhum arquivo ainda")}
          </h3>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{isEn ? "Delete file?" : "Excluir arquivo?"}</AlertDialogTitle>
            <AlertDialogDescription>
              {isEn
