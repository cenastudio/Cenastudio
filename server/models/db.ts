...
function ensureWorkspaceForExistingUsers() {
  ...
  const findMembership = db.prepare(
    "SELECT id FROM workspace_members WHERE user_id = ? AND status = 'active' LIMIT 1",
  );
  ...
  for (const user of users) {
    if (findMembership.get(user.id)) continue;
    const workspaceName = user.name?.trim() || user.email.split("@")[0] || "Meu Studio";
    const result = insertWorkspace.run(workspaceName, user.id);
    insertMembership.run(Number(result.lastInsertRowid), user.id);
  }
}
