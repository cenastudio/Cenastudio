# POLÍTICA DE PRIVACIDADE E PROTEÇÃO DE DADOS

**Última atualização: 14 de julho de 2026**

> ⚠️ **Pendência antes da publicação legal.** Este documento só é
> juridicamente vinculante depois que os campos abaixo forem preenchidos
> com os dados reais da empresa. Recomendamos revisão por um advogado
> antes de publicar para usuários finais. Checklist (leve para o advogado):
>
> - [ ] **CNPJ** da empresa — Seção 1.1 deste arquivo (`**CNPJ:**`)
> - [ ] **Endereço** completo da sede — Seção 1.1 deste arquivo (`**Endereço:**`)
> - [ ] **Nome do Encarregado de Dados / DPO** (LGPD Art. 41) — Seção 12 (`**Nome:**`)
> - [ ] **Telefone de contato do DPO** — Seção 12 (`**Telefone:**`)
> - [ ] **Telefone geral de contato** — Seção 16 (`**Telefone:**`)
> - [ ] **Foro (cidade/estado)** para resolução de conflitos — Seção 14 (`[SUA CIDADE]/[ESTADO]`)
> - [ ] Repetir os mesmos 5 itens em `client/public/privacy-policy.html`
>       (versão servida ao público) e `client/public/terms-of-use.html`
>       (menciona "foro da comarca da sede da empresa" — confirmar se o
>       texto genérico é aceitável ou se deve citar a cidade explicitamente)
> - [ ] Confirmar se **CNPJ/Razão Social/Foro** devem ser os mesmos em
>       `PRIVACY_POLICY.md` e em `client/public/privacy-policy.html`, ou se
>       um dos dois documentos deve ser descontinuado para evitar duas
>       versões divergentes da política.
> - [ ] Validar prazos de retenção (Seção 5) e a base legal de cada
>       finalidade de tratamento com o advogado antes de publicar.

---

## 1. INFORMAÇÕES GERAIS

### 1.1 Identificação do Controlador

**Razão Social:** Cena Studio
**CNPJ:** [A PREENCHER]
**Endereço:** [A PREENCHER]
**E-mail:** cenastudio@atomicmail.io
**Website:** https://cenastudio-production.up.railway.app

### 1.2 Base Legal

Esta política está em conformidade com:
- **Lei nº 13.709/2018** (Lei Geral de Proteção de Dados - LGPD)
- **Marco Civil da Internet** (Lei nº 12.965/2014)
- **Código de Defesa do Consumidor** (Lei nº 8.078/1990)

---

## 2. DADOS COLETADOS

### 2.1 Dados Fornecidos pelo Usuário

Coletamos os seguintes dados quando você se cadastra:
- **Nome completo**
- **E-mail**
- **Senha** (armazenada com criptografia bcrypt)
- **Telefone** (opcional)
- **Empresa/Produtora**
- **CNPJ** (para emissão de notas fiscais)

### 2.2 Dados de Uso do Sistema

Coletamos automaticamente:
- **Endereço IP**
- **Tipo de navegador e dispositivo**
- **Páginas acessadas e tempo de navegação**
- **Data e hora de acesso**
- **Arquivos enviados** (vídeos, imagens, documentos)

### 2.3 Dados de Projetos

Quando você usa o sistema, armazenamos:
- **Informações de clientes** (nome, e-mail, telefone, empresa)
- **Projetos de vídeo** (briefing, roteiro, orçamento)
- **Arquivos de mídia** (armazenados no Cloudinary)
- **Comentários e reviews de vídeos**
- **Histórico de interações**

### 2.4 Dados de Pagamento

**NÃO armazenamos dados de cartão de crédito.**

Pagamentos são processados pelo **Stripe**, que é certificado PCI-DSS Level 1.
Armazenamos apenas:
- **ID do cliente no Stripe**
- **Histórico de transações** (valores, datas, status)
- **Plano contratado**

---

## 3. FINALIDADE DO TRATAMENTO DE DADOS

### 3.1 Bases Legais (LGPD Art. 7)

Tratamos seus dados com base em:

**a) Execução de contrato** (Art. 7, V):
- Fornecimento do serviço SaaS contratado
- Gestão de conta e autenticação
- Processamento de pagamentos

**b) Legítimo interesse** (Art. 7, IX):
- Segurança da plataforma
- Prevenção de fraudes
- Melhoria do serviço
- Análise de uso e performance

**c) Cumprimento de obrigação legal** (Art. 7, II):
- Emissão de notas fiscais
- Atendimento a ordens judiciais
- Conformidade tributária

**d) Consentimento** (Art. 7, I):
- Envio de e-mails de marketing (opt-in)
- Uso de cookies não essenciais

### 3.2 Finalidades Específicas

Usamos seus dados para:
- ✅ Fornecer acesso à plataforma
- ✅ Processar pagamentos e emitir NF
- ✅ Enviar notificações sobre projetos
- ✅ Suporte técnico
- ✅ Prevenir fraudes e abusos
- ✅ Cumprir obrigações legais
- ✅ Melhorar funcionalidades (com dados anonimizados)

---

## 4. COMPARTILHAMENTO DE DADOS

### 4.1 Com Quem Compartilhamos

Compartilhamos dados apenas quando necessário:

**Processadores de Pagamento:**
- **Stripe** (pagamentos) - Irlanda/EUA - Certificado PCI-DSS

**Armazenamento de Arquivos:**
- **Cloudinary** (mídia) - EUA - SOC 2 Type II

**Hospedagem:**
- **Railway** (infraestrutura) - EUA - SOC 2

**Inteligência Artificial:**
- **OpenRouter** (geração de documentos) - EUA

### 4.2 Transferência Internacional

Alguns dados podem ser transferidos para fora do Brasil:
- ✅ Todos os fornecedores têm **cláusulas contratuais padrão**
- ✅ Conformidade com **LGPD Art. 33** (transferência internacional)
- ✅ Você pode **optar por não usar funcionalidades** que envolvem transferência

### 4.3 Não Vendemos Seus Dados

**JAMAIS vendemos, alugamos ou comercializamos seus dados pessoais.**

---

## 5. SEGURANÇA DOS DADOS

### 5.1 Medidas Técnicas

Implementamos:
- 🔒 **Criptografia TLS/SSL** (HTTPS em todas as conexões)
- 🔒 **Senhas com bcrypt** (hash + salt)
- 🔒 **Autenticação JWT** com expiração
- 🔒 **Firewall e proteção DDoS**
- 🔒 **Backups diários automáticos**
- 🔒 **Logs de auditoria** de acessos

### 5.2 Medidas Organizacionais

- ✅ Acesso restrito apenas a funcionários autorizados
- ✅ Termos de confidencialidade com colaboradores
- ✅ Treinamento em segurança da informação
- ✅ Revisões periódicas de segurança

### 5.3 Incidentes de Segurança

Em caso de vazamento de dados:
- 📢 **Notificaremos a ANPD** em até 2 dias úteis (LGPD Art. 48)
- 📢 **Notificaremos você** via e-mail em até 48 horas
- 📢 **Tomaremos medidas** para mitigar danos

---

## 6. SEUS DIREITOS (LGPD Art. 18)

Você tem direito a:

### 6.1 Confirmação e Acesso
- **Confirmar** se tratamos seus dados
- **Acessar** seus dados pessoais

### 6.2 Correção e Atualização
- **Corrigir** dados incompletos ou desatualizados
- **Atualizar** informações da conta

### 6.3 Anonimização e Eliminação
- **Anonimizar** dados quando possível
- **Eliminar** dados desnecessários ou tratados com base em consentimento

### 6.4 Portabilidade
- **Exportar** seus dados em formato estruturado (JSON/CSV)
- **Transferir** para outro fornecedor

### 6.5 Informação e Revogação
- **Saber** com quem compartilhamos seus dados
- **Revogar consentimento** a qualquer momento

### 6.6 Oposição
- **Opor-se** ao tratamento baseado em legítimo interesse

### 6.7 Como Exercer Seus Direitos

📧 **E-mail:** cenastudio@atomicmail.io
📋 **Pelo sistema:** Perfil → Segurança/LGPD (dashboard de transparência
de dados e solicitações de cópia/correção/exclusão)
📞 **Prazo de resposta:** Até 15 dias

---

## 7. RETENÇÃO DE DADOS

### 7.1 Dados de Conta Ativa

Enquanto sua conta estiver ativa, mantemos todos os dados para:
- Fornecimento contínuo do serviço
- Cumprimento de obrigações contratuais

### 7.2 Após Cancelamento

Quando você cancela sua conta:
- **Imediatamente:** Acesso ao sistema é desabilitado
- **30 dias:** Dados ficam em "soft delete" (recuperação possível)
- **90 dias:** Dados são anonimizados ou excluídos permanentemente

### 7.3 Exceções Legais

Podemos reter dados além do período acima para:
- **Obrigações fiscais:** 5 anos (Código Tributário Nacional)
- **Processos judiciais:** Até conclusão do processo
- **Prevenção de fraudes:** Dados anonimizados por até 2 anos

---

## 8. COOKIES E TECNOLOGIAS DE RASTREAMENTO

### 8.1 Cookies Essenciais (Não Requer Consentimento)

- **auth_token:** Mantém você logado (sessão)
- **csrf_token:** Proteção contra ataques CSRF

### 8.2 Cookies Analíticos (Requer Consentimento)

- **Analytics:** Uso do Google Analytics (opt-in)
- **Performance:** Monitoramento de performance

### 8.3 Gerenciar Cookies

Você pode desabilitar cookies no seu navegador, mas isso pode afetar funcionalidades.

---

## 9. MENORES DE IDADE

### 9.1 Restrição de Idade

Este serviço é destinado a **maiores de 18 anos**.

Não coletamos **intencionalmente** dados de menores de 18 anos.

### 9.2 Descoberta de Dados de Menores

Se descobrirmos que coletamos dados de menores:
- ❌ **Excluiremos imediatamente**
- 📧 **Notificaremos os responsáveis**

---

## 10. ISENÇÃO DE RESPONSABILIDADE

### 10.1 Responsabilidade do Usuário

**VOCÊ É RESPONSÁVEL** por:
- ✅ Proteger suas credenciais de acesso
- ✅ Garantir conformidade LGPD dos **seus clientes**
- ✅ Obter consentimento dos titulares dos dados que você insere
- ✅ Não inserir dados sensíveis sem base legal
- ✅ Uso adequado e lícito da plataforma

### 10.2 Limitação de Responsabilidade

**NÃO NOS RESPONSABILIZAMOS** por:
- ❌ Dados inseridos incorretamente por você
- ❌ Vazamentos causados por senha fraca ou compartilhada
- ❌ Violações de LGPD por dados de **terceiros** que você insere
- ❌ Uso indevido por colaboradores autorizados por você
- ❌ Falhas em serviços de terceiros (Stripe, Cloudinary, Railway)

### 10.3 Dados de Terceiros

Se você insere dados de **seus clientes** no sistema:
- **VOCÊ é o controlador** desses dados
- **Cena Studio é apenas processador** (Art. 5º, VII LGPD)
- **Você deve ter base legal** para compartilhar esses dados conosco
- **Você deve notificar seus clientes** sobre o processamento

---

## 11. MUDANÇAS NESTA POLÍTICA

### 11.1 Notificação de Mudanças

Podemos atualizar esta política. Quando houver mudanças substanciais:
- 📧 **Notificaremos via e-mail** com 15 dias de antecedência
- 📢 **Publicaremos aviso** no sistema
- 📝 **Atualizaremos a data** no topo desta página

### 11.2 Seu Consentimento

Continuar usando o serviço após mudanças significa que você aceita a nova política.

Se não concordar, você deve **cancelar sua conta**.

---

## 12. ENCARREGADO DE DADOS (DPO)

Conforme LGPD Art. 41, nosso encarregado é:

**Nome:** [A DEFINIR]
**E-mail:** cenastudio@atomicmail.io
**Telefone:** [A DEFINIR]

---

## 13. AUTORIDADE NACIONAL (ANPD)

Se você não estiver satisfeito com nossas respostas, pode contatar:

**Autoridade Nacional de Proteção de Dados**
🌐 https://www.gov.br/anpd
📧 atendimento@anpd.gov.br

---

## 14. LEI APLICÁVEL E FORO

Esta política é regida pela legislação brasileira.

Eleito o foro de **[SUA CIDADE]/[ESTADO]** para resolução de conflitos.

---

## 15. ACEITAÇÃO

**AO USAR ESTE SERVIÇO, VOCÊ DECLARA QUE:**
- ✅ Leu e compreendeu esta política
- ✅ Concorda com o tratamento de dados conforme descrito
- ✅ É maior de 18 anos
- ✅ Tem autorização para fornecer dados de terceiros (se aplicável)

---

## 16. CONTATO

Para dúvidas sobre privacidade:

📧 **E-mail:** cenastudio@atomicmail.io
📞 **Telefone:** [A DEFINIR]
🌐 **Site:** https://cenastudio-production.up.railway.app

---

**Versão:** 1.0 (rascunho — ver nota de pendência no topo do documento)
**Data de Vigência:** A definir após revisão jurídica
